...
def load_preference_dataset(
    ...
    keep_columns: List[str] = ["prompt", "text_chosen", "text_rejected"]
) -> Dataset:
    """
    ...
    # if eval names passed in, remove data that are not for these names
    if eval_names is not None:
        eval_names = set(eval_names)
        len_before = len(dataset)
        dataset = dataset.filter(lambda example: example["name"] in eval_names)
        print(f"Filtered for names in {eval_names}, dataset reduced from {len_before} to {len(dataset)}")
